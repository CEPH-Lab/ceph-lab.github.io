---
layout: page
title: Contact
permalink: /contact/
---
<div class="subpage-banner">
  <div class="banner-left">
    <h1>Contact</h1>
  </div>
  <div class="banner-right"></div>
</div>

<div style="margin-top: 30px; font-size: 16px; line-height: 1.6;">
  <h3 style="margin-top: 0; color: #508c96;">Laboratory for Computational Epidemiology and Public Health</h3>
  
  <p>
    <strong>Department of Epidemiology and Biostatistics</strong><br>
    School of Public Health<br>
    Indiana University Bloomington<br>
    309 N Woodlawn Ave (SPH Annex)<br>
    Bloomington, IN 47408
  </p>

  <p>
    <strong>Principal Investigator:</strong> Marco Ajelli<br>
    <strong>Email:</strong> <a href="mailto:majelli@iu.edu" style="color: #508c96;">majelli@iu.edu</a>
  </p>

  <p>
    <em>For inquiries regarding open positions, please get in touch with Dr. Ajelli.</em>
  </p>
</div>

<hr style="margin: 40px 0;">

### Getting to the Lab
<div style="margin: 20px 0 30px 0;">
  <img src="/images/lab-map.png" alt="Map to CEPH Lab" style="width: 100%; height: auto; border: 1px solid #e5e5e5; box-shadow: 0 4px 15px rgba(0,0,0,0.05); display: block;">
  <p style="margin-top: 10px; font-size: 0.95em; color: #555; font-style: italic;">
    Note: The <span style="color: #508c96; font-weight: bold;">teal</span> pin shows the lab location.
  </p>
</div>

**Finding Our Building**<br>
Please note that while we are part of the School of Public Health (main building located at *1025 E 7th St*), our lab is located in the **SPH Annex** (rooms 100 and 101). The Annex is situated just across the street at **309 N Woodlawn Ave**, near the intersection of 7th Street and Woodlawn Avenue. 

**Public Transit & Biking**
* **Bus:** The lab is easily accessible via **Bloomington Transit** and the IU Campus Bus system. Several routes drop off near the Indiana Memorial Union (IMU). You can check the current [Bloomington Transit schedules](https://bloomingtontransit.com/) or the IU Mobile app for routes serving the 7th & Woodlawn area.
* **Bicycle:** If you are cycling to campus, there are bike racks conveniently located directly behind our lab building.

**Parking Information**
* **Visitor Parking:** The most convenient visitor parking is available at the Indiana Memorial Union (IMU) Pay Lot, located just a short walk south of the lab on Woodlawn Avenue. 
* **Campus Permits:** If you hold an IU parking permit, please consult the interactive [IU Campus Map](https://map.concept3d.com/?id=951) for the nearest applicable EM or CH permit lots.
* **Street Parking:** Please note that metered street parking is generally located off-campus closer to downtown Bloomington, which will require a longer walk to the lab.